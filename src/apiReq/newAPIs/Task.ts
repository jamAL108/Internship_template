...
export const createNewTask = async (formdata: any) => {
    ...
    const resultOftaskCreation: any = await supabase
        .from('votum_tasks')
        .insert([
            { name: formdata.name, priority: formdata.priority, startDate: formdata.startDate, dueDate: formdata.dueDate, status: false, workspace_id: parsedUserDetails.workspace_id },
        ])
        .select()
...
export const makeDuplicate = async (duplicatetask: any) => {
    ...
    if (parsedUserDetails !== null) {
        ...
        const { data, error }: any = await supabase
            .from('votum_tasks')
            .insert([
                { name: duplicatetask.name, priority: duplicatetask.priority, startDate: duplicatetask.startDate, dueDate: duplicatetask.dueDate, status: duplicatetask.status, workspace_id: parsedUserDetails.workspace_id, sub_tasks: duplicatetask.sub_tasks, taskContent: duplicatetask.taskContent },
            ])
            .select()
