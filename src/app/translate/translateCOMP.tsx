...
const Translate = () => {
    ...
    if (filename.endsWith('.docx')) {
      ...
      try {
        ...
        if (msg.evaluate.length !== 0) {
          ...
          router.push('/translate/evaluate')
        } else {
  ...
  return (
    <div className="w-[min(90vw,1100px)] h-auto min-h-[100px] flex flex-col justify-start items-center mb-[8rem] transDiv">
      ...
      >
        <div className="bg-[#F5F5F8]  dark:bg-[transparent] border-dashed border-[2px] border-[ #c9cbe5] flex items-center rounded-md justify-start flex-col dark:border-[rgba(252,252,252,0.2)]  max-w-[350px] h-[280px] roundebox">
          <input
            type="file"
            hidden
            accept=".docx,.pdf"
